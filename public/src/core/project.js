      ...
      function saveProject() {
        ...
        butter.cornfield.save( _id, projectData, function( e ) {
          if ( e.error === "okay" ) {
            ...
            var sequencerEvents = butter.getTrackEventsByType( "sequencer" ),
                imageEvents = butter.getTrackEventsByType( "image" ),
                mapsEvents = butter.getTrackEventsByType( "googlemap" ),
                attributionData = {};

            for ( var i = 0; i < sequencerEvents.length; i++ ) {
              sequencerEvents[ i ] = sequencerEvents[ i ].popcornOptions;
            }

            for ( var k = 0; k < imageEvents.length; k++ ) {
              if ( imageEvents[ k ].popcornTrackEvent.type === "flickr" ) {
                imageEvents[ k ] = imageEvents[ k ].popcornOptions;
              } else {
                imageEvents.splice( k, 1 );
              }
            }

            attributionData.clips = sequencerEvents;
            attributionData.images = imageEvents;

            // We don't need specific data for the maps. We just need to know if we need to attribute
            // them in general if used.
            if ( mapsEvents.length ) {
              attributionData.maps = true;
            }

            // Now Publish and get URLs for embed
            butter.cornfield.publish( _id, {
