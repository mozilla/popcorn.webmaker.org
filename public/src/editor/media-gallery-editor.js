...
define( [ "localized", "util/lang", "util/uri", "util/xhr", "util/keys", "util/mediatypes", "editor/editor",
 "util/time", "util/dragndrop", "l10n!/layouts/media-editor.html" ],
  ...
  function addPhotos( data, options ) {
    ...
    if ( data.thumbnail ) {
      thumbnailSrc = data.thumbnail.source;
    } else {
      thumbnailSrc = data.fallback.source;
    }
  ...
  function addPhotoCallback( popcornOptions, data ) {
    ...
    // Not all images (Flickr) have an "Original". Mediasync API accounts for this
    // and provides a fallback, which we will also key on instead.
    if ( data.original ) {
      source = data.original.source;
    } else {
      source = data.fallback.source;
  ...
  function pagingCallback( page ) {
    ...
    container.dataset.query = _searchInput.value;
    ...
    XHR.get( "/api/search/" + _currentContainer + "?page=" + page + "&q=" + query + "&limit=" + _LIMIT, function( data ) {
      container.innerHTML = "";

      if ( data.status === "okay" ) {
        container.dataset.total = data.total;
